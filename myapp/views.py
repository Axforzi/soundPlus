...
def serve_music(request, id_music):
    music_query = Music.objects.filter(id=id_music)
    music_query.update(views=F('views') + 1)
    music = Music.objects.filter(id=id_music).first()
    
    # SERVE MUSIC
    file_path = os.path.join(settings.STATIC_ROOT, 'musics', music.song.path)
    if not os.path.exists(file_path):
        raise Http404("Archivo no encontrado.")
    
    response = serve(request, music.song.path, document_root=os.path.join(settings.STATIC_ROOT, 'musics'))
