...
class Profile(models.Model):
    ...
    @property
    def profile_img_path(self):
        return self.profileImg.name[self.profileImg.name.find('/'):]
    
    @property
    def cover_img_path(self):
        return self.coverImg.name[self.coverImg.name.find('/'):]
...
class Album(models.Model):
    ...
    @property
    def imgPath(self):
        return self.img.name[self.img.name.find('/'):]

# MUSICS
def music_img_directory(instance, filename):
    now = datetime.now()
    path = 'myapp/static/img/music_covers/'
    name = now.strftime('%d_%m_%Y-%H_%M_%S') + filename
    return path + name

def music_directory(instance, filename):
    now = datetime.now()
    path = 'myapp/static/musics/'
    name = now.strftime('%d_%m_%Y-%H_%M_%S') + filename
    return path + name
...
class Music(models.Model):
    ...
    @property
    def musicName(self):
        return self.song.path[self.song.path.find('musics/'):]
    
    @property
    def imgPath(self):
        if self.img.name is '':
            return self.album.imgPath
        else:
            return self.img.name[self.img.name.find('/'):]
...
class List(models.Model):
    ...
    def getImg(self):
        try:
            return self.musics.all().order_by('l_musics__created_at').first().imgPath
